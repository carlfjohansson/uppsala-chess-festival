...
const PRICES = {
  'Weekend 1': 400, 'Weekend 2': 400,
  'Snabbschack': 200, 'Chess960': 200, 'Blixt': 200, 'Amatör': 200,
  'Ungdom': 100
};
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Hämta och verifiera Stripe-sessionen
    const session = await stripe.checkout.sessions.retrieve(session_id);
    if (session.payment_status !== 'paid') {
      return res.status(200).json({ ok: false, reason: 'not_paid' });
    }

    if (!process.env.RESEND_API_KEY) return res.status(200).json({ ok: true, note: 'no email key' });

    const { name, email, club, rating, tournaments: tourString, message, lang } = session.metadata || {};
    const tournaments = tourString ? tourString.split(', ') : [];
    ...
    const organizerHtml = `
      <div style="font-family:sans-serif;max-width:560px;color:#222;">
        <h2 style="color:#0d3a6e;">✅ Ny kortbetalning bekräftad – Uppsala Schackfestival 2026</h2>
        <table style="border-collapse:collapse;width:100%;">
          ...
          <tr><td style="padding:8px 12px;color:#666;border-bottom:1px solid #eee;">E-post</td><td style="padding:8px 12px;border-bottom:1px solid #eee;">${email || session.customer_email || '—'}</td></tr>
          ...
          <tr><td style="padding:8px 12px;color:#666;border-bottom:1px solid #eee;">Turneringar</td><td style="padding:8px 12px;border-bottom:1px solid #eee;">${tourString || '—'}</td></tr>
          ...
          <tr><td style="padding:8px 12px;color:#666;border-bottom:1px solid #eee;">Betalningssätt</td><td style="padding:8px 12px;border-bottom:1px solid #eee;">Kort (Stripe) ✅ bekräftad</td></tr>
          ${message ? `<tr><td style="padding:8px 12px;color:#666;">Övrigt</td><td style="padding:8px 12px;">${message}</td></tr>` : ''}
        </table>
        <p style="color:#888;font-size:12px;margin-top:16px;">Stripe session: ${session_id}</p>
      </div>`;
      ...
      <div style="font-family:sans-serif;background:#0f1722;color:#f5ecd9;padding:40px 0;">
        <div style="max-width:560px;margin:0 auto;background:#1d2a3d;border-radius:16px;overflow:hidden;">
          ...
          <div style="padding:32px;">
            <h2 style="color:#f5cb6b;margin:0 0 8px;">${isEn ? 'Registration confirmed!' : 'Anmälan bekräftad!'}</h2>
            <p style="color:#c9bfa8;margin:0 0 24px;">${isEn ? `Hi ${name}, your payment has been received.` : `Hej ${name}, din betalning är genomförd och din anmälan är bekräftad.`}</p>
            ...
            <div style="background:#243349;border-radius:10px;padding:16px 20px;margin-bottom:24px;">
              <span style="color:#8e8674;font-size:0.85rem;text-transform:uppercase;letter-spacing:0.08em;">${isEn ? 'Total paid' : 'Totalt betalt'}</span>
              <span style="color:#f5cb6b;font-size:1.4rem;font-weight:bold;float:right;">${total} kr</span>
            </div>
    ...
    const playerEmail = email || session.customer_email;
    await Promise.all([
      sendEmail(process.env.RESEND_API_KEY, 'info@uppsalachessfestival.se',
        `✅ Ny anmälan (kort): ${name} — ${tourString}`,
        organizerHtml),
      playerEmail
        ? sendEmail(process.env.RESEND_API_KEY, playerEmail,
